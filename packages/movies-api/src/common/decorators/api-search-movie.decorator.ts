...
/**
 * Custom decorator for Search Movie API endpoint
 * Combines all Swagger decorators for movie search with optional similar movies
 */
/**
 * Custom decorator for Get Movie Details by ID API endpoint
 * Combines Swagger decorators for movie details fetch
 */
export function ApiSearchMovieById() {
  return applyDecorators(
    ApiOperation({ summary: 'Get movie details by ID' }),
    ApiQuery({
      name: 'id',
      description: 'The movie ID (IMDb or TMDB)',
      example: 'tt0848228',
      type: 'string',
    }),
    ApiQuery({
      name: 'language',
      required: false,
      description: 'Language code for movie details',
      example: 'en-US',
      type: 'string',
    }),
    ApiResponse({
    ...
    ApiResponse({
      ...
      schema: {
        ...
        properties: {
          ...
          message: { type: 'string', example: 'Invalid movie ID' },
...
export function ApiSearchMovie() {
  return applyDecorators(
    ApiOperation({
      summary: 'Search for movie with optional similar movies',
      description:
        'Search for movie with option to include similar movies for each result. ' +
